...
class StocksController {
     ...
     * ```
     */
    async callFinnhubAPI(endpoint, cacheDuration) {
        if (!FINNHUB_API_KEY) {
        ...
        return new Promise((resolve, reject) => {
            ...
            const request = https_1.default.get(url, options, (res) => {
                let data = '';
                res.on('data', (chunk) => {
                    data += chunk;
                });
     ...
     */
    async processBatchedRequests(requests, batchSize = REQUEST_BATCH_SIZE) {
        const results = [];
     ...
     */
    convertFinnhubToStockData(symbol, quote, profile, candles) {
            ...
            if (quote.status === 'fulfilled' && quote.value && quote.value.c > 0) {
                ...
                if (includeHistorical) {
                    try {
                        ...
                        const candleResult = await this.callFinnhubAPI(`/stock/candle?symbol=${symbol}&resolution=D&from=${fromTimestamp}&to=${toTimestamp}`, CACHE_DURATIONS.CANDLES);
