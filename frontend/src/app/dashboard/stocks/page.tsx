...
import { ChevronRight, Search, TrendingUp, TrendingDown, Activity, ArrowUpDown, Plus, Minus, Star, DollarSign, Menu } from "lucide-react"
...
interface TransactionDialogProps {
  ...
  isOpen: boolean;
  onClose: () => void;
  onSubmit: (transaction: { symbol: string; shares: number; price: number; type: 'buy' | 'sell' }) => void;
}

const TransactionDialog: React.FC<TransactionDialogProps> = ({ stock, isOpen, onClose, onSubmit }) => {
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="bg-dark border-cream/10 text-cream max-w-sm mx-4 sm:max-w-md">
        ...
        <div className="space-y-4">
          ...
          <div>
            <Label htmlFor="shares" className="text-cream/80">Shares</Label>
            <Input
              id="shares"
              type="number"
              min="1"
          ...
          <div className="p-3 bg-cream/5 rounded-lg">
            <div className="flex justify-between items-center">
              <span className="text-cream/70">Total Value:</span>
              <span className="font-semibold text-cream">{formatPrice(totalValue)}</span>
            </div>
          </div>
...
export default function StocksPage() {
  ...
  const handleTransaction = async (transaction: { symbol: string; shares: number; price: number; type: 'buy' | 'sell' }) => {
    if (!user) {
      toast({
    ...
    try {
      ...
      await StockAPI.executeTransaction({
        userId: user.uid,
      ...
      toast({
        ...
        description: `Successfully ${transaction.type === 'buy' ? 'bought' : 'sold'} ${transaction.shares} shares of ${transaction.symbol} at ${formatPrice(transaction.price)}`,
      })
      
      // Refresh stock data to reflect portfolio changes
      await loadStockData()
    } catch (error) {
  ...
  return (
    <div className="flex min-h-screen bg-dark text-cream">
      <DashboardSidebar collapsed={collapsed} setCollapsed={setCollapsed} />
      <div className="flex-1 p-4 sm:p-6 space-y-4 sm:space-y-6 min-w-0 overflow-x-auto">
        {/* Header */}
