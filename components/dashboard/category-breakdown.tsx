...
const categoryData = [
  { name: "Dining", value: 840, color: "#F5F5F0" },
  { name: "Shopping", value: 620, color: "rgba(245, 245, 240, 0.8)" },
  { name: "Transport", value: 480, color: "rgba(245, 245, 240, 0.6)" },
  { name: "Utilities", value: 350, color: "rgba(245, 245, 240, 0.4)" },
  { name: "Other", value: 280, color: "rgba(245, 245, 240, 0.2)" },
]
        ...
        <div className="h-full flex flex-col">
          ...
          <div className="flex justify-between items-center p-4 border-b border-cream/10">
            ...
            <button
              ...
              className="text-cream/60 hover:text-cream flex items-center"
            >
          ...
          <div className="flex-1 p-6 overflow-hidden flex flex-col">
            ...
            <div className="mb-6">
              <div className="relative inline-block">
                <button 
                  ...
                  className="flex items-center bg-cream/5 rounded-lg px-3 py-2 text-sm hover:bg-cream/10"
                >
                ...
                {showDateFilter && (
                  <div className="absolute top-full left-0 mt-1 bg-[#1E1E1E] border border-cream/10 rounded-lg shadow-lg py-1 z-10 w-full">
                    {dateRangeOptions.map((option) => (
                      <button
                        key={option.value}
                        className={`w-full text-left px-4 py-2 text-sm hover:bg-cream/5 ${dateRange === option.value ? 'text-cream font-medium' : 'text-cream/70'}`}
                        onClick={() => {
            ...
            <div className="flex-1 flex gap-6 overflow-hidden">
                    ...
                    <BarChart data={categoriesWithPercentages}>
                      ...
                      >
                        {categoryData.map((entry, index) => (
                          <Cell 
                            key={`cell-${index}`} 
                            fill={selectedCategory === entry.name ? entry.color : `${entry.color}99`}
                            cursor="pointer"
                            onClick={() => selectCategory(entry.name)}
                          />
                        ))}
              ...
              <div className="w-2/5 overflow-hidden">
                ...
                <div className="space-y-4 overflow-y-auto h-[calc(100%-32px)] pr-2">
                  {categoriesWithPercentages.map((category, index) => (
                    <div 
                      key={index} 
                      className={`bg-cream/5 rounded-lg p-4 cursor-pointer transition-all duration-300 ${selectedCategory === category.name ? 'border border-cream/40' : 'hover:bg-cream/10'}`}
                        ...
                        <div className="mt-4 pt-4 border-t border-cream/10 space-y-3">
                          <div className="grid grid-cols-2 gap-4">
                            <div className="bg-cream/5 rounded-lg p-3">
                              ...
                              <div className="text-xl font-medium">${category.value}</div>
                            </div>
                            <div className="bg-cream/5 rounded-lg p-3">
                              <div className="text-xs text-cream/60">% of Budget</div>
                              <div className="text-xl font-medium">{category.percentage}%</div>
                            </div>
                          </div>
                          ...
                          <div className="space-y-2 max-h-[200px] overflow-y-auto">
                            {transactionsByCategory[category.name]?.map((transaction: Transaction, idx: number) => (
                              <div key={idx} className="bg-cream/5 rounded-lg p-2 flex items-center justify-between">
                                <div className="flex items-center">
                                  <div className="h-8 w-8 rounded-full bg-cream/10 flex items-center justify-center mr-3">
                  ...
                  <div className="mt-4 pt-4 border-t border-cream/10 flex justify-between items-center">
  ...
  return (
    <div className="bg-transparent">
      ...
      <div className="mt-4">
        ...
        <button
          onClick={toggleExpanded}
          className="mt-2 w-full text-center text-xs text-cream/60 hover:text-cream flex items-center justify-center group category-breakdown-expand-btn"
        >
